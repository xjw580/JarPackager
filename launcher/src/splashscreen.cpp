...
SplashScreen::SplashScreen(const std::vector<char>& pngData,
                           const std::wstring& programName,
                           const std::wstring& programVersion)
    : m_hwnd(nullptr),
      m_width(400), m_height(300),
      m_programName(programName), m_programVersion(programVersion),
      m_progress(0), m_statusText(L"正在初始化..."),
      m_gdiplusBitmap(nullptr), m_cachedBitmap(nullptr),
      m_progressHeight(0), m_autoProgress(false),
      m_progressStep(0.5), m_progressInterval(50), m_autoCloseDelay(0) {
    ...
    // 注册窗口类
    const wchar_t* className = L"SplashScreenClass";
    WNDCLASSEXW wc = {};
...
void SplashScreen::CreateBitmapFromPNG(const std::vector<char>& pngData) {
    ...
    int targetHeight = static_cast<int>(screenHeight / 3.0f); // 屏幕高度的1/3
...
void SplashScreen::CreateDefaultBackground() {
    ...
    LinearGradientBrush gradientBrush(
        Point(0, 0), Point(0, m_height),
        Color(255, 30, 60, 120),   // 起始颜色
...
void SplashScreen::CalculateLayout() {
    ...
    m_titleRect = RectF(
        margin,
        static_cast<float>(m_height) / 3.0f,
        static_cast<float>(m_width) - 2 * margin,
        50.0f * dpiScale
    );
    ...
    m_versionRect = RectF(
        margin,
        m_titleRect.Y + m_titleRect.Height + 10.0f * dpiScale,
        static_cast<float>(m_width) - 2 * margin,
        30.0f * dpiScale
    );

    // 进度条位置（图片底部）
    m_progressRect = RectF(
        0,
        static_cast<float>(m_height - m_progressHeight),
    ...
    m_statusRect = RectF(
        margin,
        m_progressRect.Y - 35.0f * dpiScale,
        static_cast<float>(m_width) - 2 * margin,
        25.0f * dpiScale
    );
}
...
void SplashScreen::DrawToCachedBitmap() {
    if (!m_cachedBitmap || !m_gdiplusBitmap) return;
    ...
    if (!m_programName.empty()) {
        ...
        Font titleFont(&fontFamily, 42.0f * dpiScale, FontStyleBold, UnitPixel);
    ...
    if (!m_programVersion.empty()) {
        FontFamily fontFamily(L"Microsoft YaHei");
        Font versionFont(&fontFamily, 24.0f * dpiScale, FontStyleRegular, UnitPixel);
...
void SplashScreen::SetAutoCloseDelay(DWORD delayMs) {
...
void SplashScreen::UpdateDisplay() {
    if (!m_hwnd || !m_cachedBitmap) return;
    ...
    if (!m_statusText.empty()) {
        FontFamily fontFamily(L"Microsoft YaHei");
        Font statusFont(&fontFamily, 15.0f * dpiScale, FontStyleRegular, UnitPixel);
    ...
    // 绘制进度条
    if (m_progress > 0) {
        float barWidth = static_cast<float>(m_width) * static_cast<float>(m_progress) / 100.0f;
    ...
    UpdateLayeredWindow(m_hwnd, hdcScreen, &ptDst, &size,
